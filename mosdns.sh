...
if [ "$select" = 3 ]; then
  ...
  wget https://github.com/IrineSistiana/mosdns/releases/latest/download/mosdns-linux-arm64.zip -O /tmp/mosdns-update/mosdns.zip
...
if [ "$select" = 4 ]; then
  version=$(curl -sSL -H "Accept: application/vnd.github.v3+json" "https://api.github.com/repos/771073216/geofile/releases/latest" | awk -F '"' '/tag_name/ {print $4}')
...
if [ "$select" = "install" ]; then
  ...
  wget https://github.com/IrineSistiana/mosdns/releases/latest/download/mosdns-linux-arm64.zip -O /tmp/mosdns-install/mosdns.zip
